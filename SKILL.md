...
- ✅ **パックは「順序・全体像」に答える形にする**:
  - **どの順？** → フェーズは**標準順（着手→立ち上げ→計画→実行→デリバリー→クローズ）**で並べる。
  - **全体像？** → 6フェーズをすべて置き俯瞰できる。
  - パックは**方法論の参照（テンプレ）**であり、案件の進捗管理は持たない（進捗は Jira/Notion 等の担当領域）。
...
同じ22プロセスでも、縦の切り方で読みやすさが激変する：

| | ❌ 失敗（機能サイロで縦を切る） | ✅ 正解（仕事の種類で縦を切る） |
|---|---|---|
| 縦の行 | PM / Salesforce / DMP / アプリ / BI / インフラ | 要件定義 / 設計 / 開発・構築 / テスト / PM・統制 |
| 「要件定義」を見たい人 | SF行・アプリ行・BI行…に**散らばり縦読みを強いられる** | **1行を左→右に読むだけ** |
| 行の意味 | 「どのチーム/製品の話か」 | 「この仕事が時間とともにどう進むか」 |

見分け方: 行の名前が**組織図や製品名に見えたら失敗**。「◯◯する」と言い換えられる名前（要件を定義する／設計する／テストする）なら正解。
...
1. **素材を理解する**：ユーザーのドキュメント/議事録/手順を読み、「どんな案件・方法論か」「登場する仕事の種類」「時間の流れ」を掴む。**素材が薄いときに聞くのは3つだけ**：①どんな案件・方法論？（1行で）②登場する仕事の種類は？（要件定義/設計/テスト…）③一番伝えたい失敗談・コツは？（→ `why` の種になる）。それ以上は聞かず、標準スケルトン（§7）で提案して進める。
2. **マトリクスを決める**：
...
### 納品前セルフチェック（全部 YES になるまで直してから報告する）

- [ ] 縦は「仕事の種類」か？（チーム名・製品名の行が無いか → §2 失敗例）
- [ ] 各ディシプリンは1行に収まり、左→右で時間が流れるか？（縦読みを強いていないか）
- [ ] フェーズは標準6を標準順で置いたか？
- [ ] 全タスクに `why` と `how` があるか？（空箱ゼロ。骨子のみは `status:"draft"` で明示）
- [ ] 素材に無い数値・固有名詞・手順を書いていないか？
- [ ] ファイルは `suppliedAssets` だけか？
- [ ] `visibility:"private"` のままか？（公開・共有はユーザー確認後）
- [ ] `get_pack web-waterfall` と並べて、同じ"文法"に見えるか？

報告時は「パックURL＋作った/変えたこと＋このチェック結果＋残課題」を簡潔に。**AI起草・要レビューである旨を必ず添える**。
